...
class AutologuotSettingsForm extends ConfigFormBase {
  ...
  public function validateForm(array &$form, array &$form_state) {
    ...
    if (!is_numeric($timeout) || ((int) $timeout != $timeout) || $timeout < 60 || $timeout > $max_timeout) {
      form_set_error('autologout_timeout',$form_state, t('The timeout must be an integer greater than 60 and less then %max.', array('%max' => $max_timeout)));
    }
  ...
  public function submitForm(array &$form, array &$form_state) {
    ...
    \Drupal::config('autologout.settings')
      ->set('autologout_timeout', $form_state['values']['autologout_timeout'])
      ->set('autologout_max_timeout', $form_state['values']['autologout_max_timeout'])
      ->set('autologout_padding', $form_state['values']['autologout_padding'])
      ->set('autologout_role_logout', $form_state['values']['autologout_role_logout'])
      ->set('autologout_redirect_url', $form_state['values']['autologout_redirect_url'])
      ->set('autologout_no_dialog', $form_state['values']['autologout_no_dialog'])
      ->set('autologout_message', $form_state['values']['autologout_message'])
      ->set('autologout_inactivity_message', $form_state['values']['autologout_inactivity_message'])
      ->set('autologout_use_watchdog', $form_state['values']['autologout_use_watchdog'])
      ->set('autologout_enforce_admin', $form_state['values']['autologout_enforce_admin'])
      ->save();
    foreach ($form_state['values']['table'] as $user) {
    ...
    if (isset($form_state['values']['autologout_jstimer_format'])) {
      \Drupal::config('autologout.settings')
